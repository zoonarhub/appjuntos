...
const TIPO_COLORS: Record<string, string> = {
  'Lideranca Geral': 'badge-primary',
  'Lideranca Regional': 'badge-purple',
  'Lideranca Local': 'badge-info',
...
const Liderancas: React.FC = () => {
  ...
  const fetchPessoas = async () => {
    setLoading(true);
    const { data, error } = await supabase.from('liderancas').select('*').order('created_at', { ascending: false });
    if (!error && data) {
      setPessoas(data);
    }
    setLoading(false);
  ...
  const handleDelete = async (id: string, e: React.MouseEvent) => {
    ...
    if (!error) {
      addNotification('Pessoa removida', 'success');
      fetchPessoas();
  ...
  const openEdit = (pessoa: any, e?: React.MouseEvent) => {
    if (e) e.stopPropagation();
    setFormData({
      nome: pessoa.nome || '',
      cpf: pessoa.cpf || '',
      tipo: pessoa.tipo || 'Liderança',
  ...
  const handleExport = () => {
    addNotification('Exportando base de pessoas...', 'info');
  ...
  }), [search, tipoFilter, statusFilter, pessoas]);
    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">{pessoas.length} registros • {filtered.length} exibindo</p>
        </div>
      ...
      <div className="grid-4 mb-lg" style={{ marginBottom: 'var(--space-lg)' }}>
        {[
          { label: 'Total Cadastros', value: pessoas.length, color: '#6366F1' },
          { label: 'Ativos', value: pessoas.filter(p => p.status === 'ativo').length, color: '#10B981' },
          { label: 'Inativos', value: pessoas.filter(p => p.status === 'inativo').length, color: '#EF4444' },
          { label: 'Liderancas', value: pessoas.filter(p => p.tipo?.includes('Lideranca')).length, color: '#F59E0B' },
        ].map((s, i) => (
          ...
          <div className="modal modal-lg" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              <h2 className="modal-title">
                {!isEditing && selectedPerson ? 'Detalhes da Liderança' : (selectedPerson ? 'Editar Liderança' : 'Nova Liderança')}
              </h2>
